                ...
                <form action="{{ route('roles.update', $data['role']->id) }}" class="form-validate" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            @php
                                $upper_roles = $data['role']->upper_roles??[];
                            @endphp
                            <div class="col-md-12 mt-3">
                                <div class="form-group">
                                    <label class="form-label" for="fv-email">{{ _trans('common.Upper Roles') }} <span
                                            class="text-danger">*</span></label>
                                    <div class="form-control-wrap">
                                        <select name="upper_roles[]" multiple id="upper_roles" class="form-select select2-input ot-input mb-3 modal_select2" required>
                                            <option value="" disabled>{{ _trans('common.Choose One') }}</option>
                                            @foreach ( $data['roles']->where('id','!=',$data['role']->id) as $role)
                                                <option value="{{@$role->id}}" {{in_array($role->id,$upper_roles)?'selected':''}}>{{@$role->name}}</option>
                                            @endforeach
