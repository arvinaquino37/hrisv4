...
function attendanceDatatable(...values) {
  ...
  let { from, to } = __date_range ?? {};
  var shortBy = $("#short_by").val();
  var is_detail_report = $('#customSwitch1:checked').val() ?? '0';
  data["value"] = {
    from: from ? from : null,
    to: to ? to : null,
    short_by: shortBy ? shortBy : null,
    ...
    type: $("#type").val(),
    _token: _token,
    is_detail_report : is_detail_report,
  };
  data["column"] = [
    "id",
    "name",
...
$(".conference_table_class").length > 0 && awardTable();
...
//Visit reports data table start
