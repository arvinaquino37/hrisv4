...
class DashboardController extends Controller
{
    ...
    public function loadMyProfileDashboard($request)
    {
        try {
            ...
            $noOut = Attendance::with('User')  // ['places', 'users.people']
                            ->select('*')
                            ->where('date', '=', $dateNow)
                            ->whereNull('check_out')
                            ->groupBy('user_id')
            ...
            $countIn = Attendance::where('date', '=', $dateNow)
                            ->whereNull('check_out')
                            ->count(DB::raw('DISTINCT user_id'));

            return $returnHTML = view('backend.dashboard.load_my_dashboard', compact('data', 'noOut', 'countIn'))->render();
    ...
    public function loadCompanyDashboard($request) //Timekeeper Dashboard
    {
        try {
            $request['month'] = date('Y-m');
