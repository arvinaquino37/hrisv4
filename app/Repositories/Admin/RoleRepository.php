...
class RoleRepository
{
    ...
    {
        return $this->role->query()->where('company_id', $this->companyInformation()->id)->where('status_id', 1)->where('id', '!=', '1')
        ->where('permissions', '!=', '')
        ->whereNotIn('id', auth()->user()->role->upper_roles??[])
        ->get();
    }
    ...
    {
        $this->role->query()->where('id', $request->role_id)->update($request->only(['name', 'status_id', 'permissions','upper_roles']));
        return true;
    }
