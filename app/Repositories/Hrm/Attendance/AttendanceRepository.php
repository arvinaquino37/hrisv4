...
class AttendanceRepository
{
    ...
    public function userAttendance($user, $request)
    {
	    ...
	    $duplicateAttendance = $this->attendance->where('user_id', $user->id)->where( 'date','>=', date('Y-m-d H:i:s',strtotime("-1 days")))->whereNull('check_out')->first();
            ...
            if ($isIpRestricted) {
                ...
                if (count($attendance_status) > 0) {
                    ...
                    if ($request->check_in_image) {
                        $check_in->check_in_image = $this->uploadImage($request->check_in_image, 'uploads/attendence/check_in')->id;
                    }
                    $check_in->save();
                    ...
                    if (count($attendance_status) > 0) {
                        ...
                        if ($check_in) {
                            ...
                            if ($request->check_out_image) {
                                $check_in->check_out_image = $this->uploadImage($request->check_out_image, 'uploads/attendence/check_out')->id;
                            }
                            $check_in->save();
                            if ($request->reason) {
                                LateInOutReason::create([
