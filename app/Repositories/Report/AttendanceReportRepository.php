...
class AttendanceReportRepository
{
        ...
        return datatables()->of($monthArray)
            ...
            ->addColumn('checkin', function ($data) use ($user) {
                ...
                if (in_array($todayDateName, $weekEnds)) {
                    return '<span class="badge badge-info ml-2" data-toggle="tooltip" data-placement="top"> Weekend </span>';
                }
                ...
                if ($holidays != '') {
                    return '<span class="badge badge-info ml-2" data-toggle="tooltip" data-placement="top"> Holiday </span>';
                }
                ...
                if ($leaveDate != '') {
                    return '<span class="badge badge-info ml-2" data-toggle="tooltip" data-placement="top"> Leave </span>';
                }
                ...
                if ($attendance) {
                    ...
                    return '<span class="badge badge-dark ml-2" data-toggle="tooltip" data-placement="top"> Absent </span>';
                }
            })
        ...
        if ($data->check_out) {
            ...
            if (is_array($checkOutStatus)) {
                if ($checkOutStatus[0] === 'LT') {
                    ...
                    $status = '';
                    $status .= '<div class="d-flex badge-responsive">';
                    $status .= $data->check_out ? '<span class="badge badge-primary">' . $this->dateTimeInAmPm($data->check_out) . '</span>' : '';
                    $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $data->check_out_location . '"> <i class="fa fa-h-square"></i> </span>';
                    $status .= '</div>';
                    return $status;
                }
    ...
    function table($request)
    {
        ...
        if ($request->is_detail_report) {
            //custom changes made by cijagani
          return [
            'data' => $data->groupby(function($data){
               return $data->date.'-'.$data->user_id;
            })->map(function ($group) {
                $check_in = $this->CheckInTable($group->first());
                $check_out = $this->checkOutTable($group->last());
                $action_button = '';
                if (hasPermission('attendance_update')) {
                    $action_button .= actionButton(_trans('common.Edit'), route('attendance.checkInEdit', $group->pluck('id')->first()), 'profile');
                }
                ...
                return [
                    ...
                    'name'          => '<a href="' . route('employeeAttendance', $group->first()->user->id) . '" target="_blank">' . @$group->first()->user->name . '</a>',
                    'department'    => @$group->first()->user->department->title,
                    'totalBreak'    =>  RawTable('employee_breaks')->where(['date' => $group->pluck('date')->first(), 'user_id' => $group->pluck('user_id')->first()])->count(),
                    'breakDuration' => breakDuration($group->pluck('date')->first(), $group->pluck('user_id')->first()),
                    'checkin'       => $check_in,
                    'checkInImage'  => TagImgShow($group->pluck('check_in_image')->first(),$group->pluck('id')->first(),'check_in'),
                    'checkout'      => $check_out ?? '',
                    'checkOutImage' => TagImgShow($group->pluck('check_out_image')->first(),$group->pluck('id')->first(),'check_out'),
                    'hours'         => (@$group->pluck('check_out')->first()) ? $this->timeDifference($group->pluck('check_in')->first(), $group->pluck('check_out')->last()) : '',
                    'overtime'      => $this->overTimeCount($group->first()) ?? '',
                    'date'          =>  $group->pluck('date')->first(),
                    'status'        => '<small class="badge badge-' . @$group->first()->status->class . '">' . @$group->first()->status->name . '</small>',
                    'action'        => $button,
                ];
            })->values(),
            ...
            'data' => $data->map(function ($data) {
                ...
                return [
                    ...
                    'checkInImage'  => TagImgShow($data->check_in_image,$data->id,'check_in'),
                    'checkout'      => $this->checkOutTable($data) ?? '',
                    'checkOutImage' => TagImgShow($data->check_out_image,$data->id,'check_out'),
